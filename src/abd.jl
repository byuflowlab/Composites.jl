...
getQ(mat::material,theta::Float64=0.0) = getQ(mat.e1,mat.e2,mat.g12,mat.nu12,theta)
...
"""
    ...
    getQ(mat::Array{material,1},theta::Array{Float64,1}=zeros(Float64,length(mat)))
    getQ(mat::Array{material,1},lam::laminate)
Returns multiple Q matrices. Third dimension corresponds to the ply number.
"""
function getQ(e1::Array{Float64,1},e2::Array{Float64,1},g12::Array{Float64,1},
  nu12::Array{Float64,1},theta::Array{Float64,1}=zeros(Float64,length(e1)))
  nply = length(e1)
...
function getQ(mat::Array{material,1},theta::Array{Float64,1}=zeros(Float64,length(mat)))
  nply = length(mat)
  qbar = zeros(Float64,3,3,nply)
  for i = 1:nply
...
"""
    getABD(nply::Array{Int64,1},tply::Array{Float64,1},matid::Array{Int64,1},
    theta::Array{Float64,1},e1::Array{Float64,1},e2::Array{Float64,1},
    g12::Array{Float64,1},nu12::Array{Float64,1})
    getABD(lam::laminate,q::Array{Float64,3}) = getABD(lam.matid,lam.nply,lam.tply,lam.theta,q)
...
* `theta::Array{Float64,1}`: orientation (deg) of each lamina
...
"""
function getABD(matid::Array{Int64,1},nply::Array{Int64,1},tply::Array{Float64,1},
