...
@testset "getplystrain/getplystress" begin
  ...
  @test (lowerplystrain,upperplystrain) == Composites.getplystrain(nply,tply,theta,resultantstrain)
  ...
  for i = 1:size(lowerplystress,1)
    for j = 1:size(lowerplystress,2)
      @test round(lowerplystress[i,j]/1e6,3) == lowerplystresstest[i,j]
    end
  end
  # getplystress(plystrain::Array{Float64,2},q::Array{Float64,3},lam::laminate) = getplystress(plystrain,q,matid)
  ...
  for i = 1:size(upperplystress,1)
    for j = 1:size(upperplystress,2)
      @test round(upperplystress[i,j]/1e6,3) == upperplystresstest[i,j]
    end
  end
end
...
@testset "maxstress" begin
 ...
 -17.7378     -2.49131    5.23433  -53.2135   -2.27273   1.53764   13.3034;
  ...
  failmaxstress,sfmaxstress = Composites.maxstress(upperplystress,mat,lam)
  for i = 1:6
    for j = 1:7
      @test signif(failmaxstress[i,j],6) == failmaxstresstest[i,j]
      @test signif(sfmaxstress[i,j],6) == sfmaxstresstest[i,j]
    end
  end
end
...
@testset "tsaiwu" begin
  ...
  failtsaiwu,sftsaiwu = Composites.tsaiwu(upperplystress,mat,lam)
  for i = 1:7
      @test signif(failtsaiwu[i],6) == failtsaiwutest[i]
      @test signif(sftsaiwu[1,i],6) == sftsaiwutest[1,i]
      @test signif(sftsaiwu[2,i],6) == sftsaiwutest[2,i]
  end
end
...
@testset "hashinrotem" begin
  ...
  failhashinrotem,sfhashinrotem = Composites.hashinrotem(lowerplystress,mat,lam)
  for i = 1:4
    for j = 1:7
      @test signif(failhashinrotem[i,j],6) == failhashinrotemtest[i,j]
      @test signif(sfhashinrotem[i,j],6) == sfhashinrotemtest[i,j]
    end
  end
end
