...
function clt()
    ...
    @testset "tsaiwu" begin
        ...
        for i = 1:7
            @test round(failtsaiwu[i], sigdigits=6) == failtsaiwutest[i]
            @test round(sftsaiwu[i][1], sigdigits=6) == sftsaiwutest[1,i]
            @test round(sftsaiwu[i][2], sigdigits=6) == sftsaiwutest[2,i]
        end
    end
